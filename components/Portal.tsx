    ...
    try {
      if (demo) {
        ...
        const { data, error } = await createClient()
          ...
          .single();
        if (error) throw error;
        if (!data) throw new Error("The job status was not saved.");
        await loadData();
      }
      show(
        "success",
        `${job.job_number} changed to ${labels[nextStatus] || nextStatus}`,
      );
    } catch (error) {
      show(
        "error",
          ...
          {jobs.map((j) => {
            ...
            <tr key={j.id} onClick={() => open(j)}>
              ...
              <td data-label="Status">
                <select
                  className={`statusSelect ${j.status}`}
                  value={j.status}
                  aria-label={`Change status for ${j.job_number}`}
                  onClick={(event) => event.stopPropagation()}
                  onChange={(event) => {
                    event.stopPropagation();
                    void changeStatus(j, event.target.value as JobStatus);
                  }}
                >
                  {STATUSES.filter(
                    (jobStatus) =>
                      jobStatus !== "unpaid" && jobStatus !== "cancelled",
                  ).map((jobStatus) => (
...
function Tasks({
  ...
  const hiddenJobStatuses: JobStatus[] = ["completed", "cancelled"];
  const invoicedJobIds = new Set(
    documents
      .filter((document) => document.document_type === "invoice" && document.job_id)
      .map((document) => document.job_id),
  );
  return (
    <>
      <section className="sectionHead">
        ...
        {profiles
          ...
          .map((p) => {
            const assignedJobs = jobs.filter(
              (j) =>
                j.assigned_admin_id === p.id &&
                !hiddenJobStatuses.includes(j.status),
            );
            const assignedTasks = tasks.filter(
              (task) => {
                if (task.assigned_to !== p.id || task.status === "done") return false;
                const job = jobs.find((item) => item.id === task.job_id);
                if (job && hiddenJobStatuses.includes(job.status)) return false;
                return !(
                  task.title === "Create invoice and email customer" &&
                  task.job_id &&
                  invoicedJobIds.has(task.job_id)
                );
              },
            );
            return (
