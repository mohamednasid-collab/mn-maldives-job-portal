...
export interface DocumentItem { id?: string; item_id?: string | null; description: string; detail: string | null; quantity: number; rate: number; position: number; }
export interface FinancialDocument { id: string; document_type: "quotation" | "invoice"; document_number: string; status: "draft" | "sent"; source_quotation_id: string | null; job_id: string | null; customer_name: string; customer_address: string | null; subject: string | null; issue_date: string; due_date: string | null; terms: string; discount_percent: number; advance_payment: number; amount_paid: number; voided_at: string | null; void_reason: string | null; voided_by: string | null; notes: string | null; created_at: string; items: DocumentItem[]; }
export interface Payment { id: string; invoice_id: string; payment_date: string; amount: number; payment_method: string | null; reference: string | null; notes: string | null; created_at: string; invoice?: Pick<FinancialDocument, "id" | "document_number" | "customer_name" | "job_id" | "discount_percent" | "advance_payment" | "amount_paid" | "voided_at" | "items"> | null; }
...
export interface Job {
  id: string; job_number: string; customer_id?: string | null; customer_name: string; customer_phone: string | null; customer_email?: string | null; contact_person?: string | null;
